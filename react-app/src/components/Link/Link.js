...
export default function Link({
    ...
    const getLocationRun = async (position) => {
        ...
        console.log("CSRF TOKEN I (Link.js)");
        console.log({ csrfToken });

        fetch((process.env.NODE_ENV === 'production'
            ? '/app'
            : 'http://localhost:5001') + '/api/locale/optional', {
            method: 'post',
            headers: {
    ...
    const getLocationError = async (error) => {
        alert(
        ...
        console.log("CSRF TOKEN II (Link.js)");
        console.log({ csrfToken });
