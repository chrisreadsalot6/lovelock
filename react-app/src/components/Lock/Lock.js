...
export default function Lock({ isMobile, joeColor, revealJoe, user }) {
    ...
    const getGeolocationData = async () => {
        ...
        fetch(
            (process.env.NODE_ENV === 'production'
                ? '/app'
                : 'http://localhost:5001') + `/api/lock/${lockId}/get-geolocation`,
            {
                method: 'get',
                headers: {
    ...
    const pullCompassData = () => {
        fetch(
            (process.env.NODE_ENV === 'production'
                ? '/app'
                : 'http://localhost:5001') + `/api/lock/${lockId}/pull-compass`,
            {
                method: 'get',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const pushAndPullData = async () => {
        ...
        fetch(
            (process.env.NODE_ENV === 'production'
                ? '/app'
                : 'http://localhost:5001') + `/api/lock/${lockId}/push-compass`,
            {
                method: 'post',
                headers: {
                    'Content-Type': 'application/json',
